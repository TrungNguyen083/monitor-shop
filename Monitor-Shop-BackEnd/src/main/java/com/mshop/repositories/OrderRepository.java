...
public interface OrderRepository extends JpaRepository<Order, Long> {
	...
	@Query(value = "select year(order_date) from orders group by year(order_date) ORDER BY year(order_date) DESC", nativeQuery = true)
	List<Integer> getYears();

	@Query(value = "SELECT p.product_id AS productId, p.name AS productName, COUNT(*) AS amount, p.price*COUNT(*) AS totalPrice " +
			"FROM `monitor-shop`.order_details od " +
